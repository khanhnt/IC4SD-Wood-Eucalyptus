...
def scan_raw_dataset(
    ...
    if compute_phash and imagehash is None:
        print("[WARN] imagehash is not installed; pHash values will not be computed.", flush=True)
        ...
        for path in tqdm(all_files, desc=f"Scanning {class_dir.name}", leave=False):
            ...
            try:
                with Image.open(path) as image:
                    ...
                    if compute_phash and imagehash is not None:
            ...
            rows.append(
                {
                    ...
                    "sha256": file_hash,
                    "phash": phash,
                    "parsed_group_id": parse_group_id(relative_path, class_name),
